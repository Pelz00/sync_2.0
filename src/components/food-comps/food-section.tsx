...
import PastaImage from '@/assets/images/illustrtions/local-food-illustration.png'
...
import LocalFoodImage from '@/assets/images/illustrtions/swallow-illustration.png'
...
import AllImage from '@/assets/images/illustrtions/plate-illustration.png'
...
export default function FoodSection({ food }: Props) {
    ...
    // illustration tabs (multi-select, "All" = no filter)
    ...
    if (foodTypes.length > 0) {
        filtered = filtered.filter(f => foodTypes.includes(f.category))
    }
    ...
    const hasActiveFilters =
        promotionsOn || topRatedOn || foodTypes.length > 0 || sortBy !== "recommended"

    const isAllActive = activeTabs.length === 0
    ...
    return (
        <>
            ...
            <div className="mt-4 flex gap-3 overflow-x-auto scrollbar-none pb-3">
                {ILLUS_TABS.map(tab => {
                    const active = tab.label === "All" ? isAllActive : activeTabs.includes(tab.label)
                                ...
                                {active && tab.label !== "All" && (
                                    <div className="absolute bottom-0 right-0 w-5 h-5 rounded-full bg-ink border-2 border-surface flex items-center justify-center z-10">
                                        <svg width="9" height="7" viewBox="0 0 10 8" fill="none">
                                            <path d="M1 4L3.5 6.5L9 1" stroke="#C5FF4A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                                        </svg>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="flex gap-1.5 mt-1.5 flex-wrap">
                        {promotionsOn && (
                            <span className="flex items-center gap-1 bg-lime/15 text-lime-deep dark:text-lime text-[11px] font-medium px-2.5 py-0.5 rounded-full border border-lime/20">
                                Promotions
                                <button onClick={() => setPromotionsOn(false)} className="cursor-pointer"><X size={10} /></button>
                            </span>
                        )}
                        {topRatedOn && (
                            <span className="flex items-center gap-1 bg-lime/15 text-lime-deep dark:text-lime text-[11px] font-medium px-2.5 py-0.5 rounded-full border border-lime/20">
                                Top Rated
                                <button onClick={() => setTopRatedOn(false)} className="cursor-pointer"><X size={10} /></button>
                            </span>
                        )}
                        {foodTypes.map(t => (
                            <span key={t} className="flex items-center gap-1 bg-lime/15 text-lime-deep dark:text-lime text-[11px] font-medium px-2.5 py-0.5 rounded-full border border-lime/20">
                                {t}
                                <button onClick={() => setFoodTypes(prev => prev.filter(x => x !== t))} className="cursor-pointer"><X size={10} /></button>
                            </span>
                        ))}
                    </div>
                )}
                    ...
                    <div className="bg-panel w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl border border-content-muted/20 shadow-[0_-8px_32px_rgba(0,0,0,0.15)] flex flex-col max-h-[85vh]">
                        <div className="flex items-center justify-between px-5 py-4 border-b border-content-muted/20 flex-shrink-0">
                            ...
                            <button
                                ...
                                className="w-8 h-8 rounded-full border border-content-muted/30 flex items-center justify-center text-content-muted hover:text-content cursor-pointer"
                            >
                                <X size={16} />
                            </button>
                        </div>
                        ...
                        <div className="overflow-y-auto flex-1 px-5 py-4">
                            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                                {FOOD_TYPE_CATEGORIES.map(({ label, image }) => {
                        ...
                        <div className="px-5 py-3 flex flex-col gap-1">
                            {SORT_OPTIONS.map(opt => (
                                <button
                                    ...
                                    className="flex items-center justify-between w-full px-4 py-3.5 rounded-xl hover:bg-content-muted/5 cursor-pointer transition"
                                >
