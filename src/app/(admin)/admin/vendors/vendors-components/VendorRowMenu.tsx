...
import { MoreVertical, Eye, CheckCircle, PauseCircle, Trash2, ShieldCheck, X } from "lucide-react";
  ...
  return (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            className="h-8 w-8 rounded-md text-content-muted hover:text-content hover:bg-surface-deep/80 cursor-pointer ">
        ...
        <DropdownMenuContent
          ...
          {vendor.status !== "Active" && (
            <DropdownMenuItem
              ...
              className="flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-lime rounded-lg cursor-pointer transition-colors">
          ...
          {vendor.status !== "Suspended" && (
            <DropdownMenuItem
              onClick={(e) => { e.stopPropagation(); setSuspendOpen(true); }}
      ...
      <SuspendModal
        ...
        onConfirm={(reason) => { onSuspend(vendor, reason); setSuspendOpen(false); }}
      />
    </>
  );
}